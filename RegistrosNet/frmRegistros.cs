...
namespace RegistrosNet
{
    ...
    public partial class frmRegistros : Form
    {
        ...
        private void EjecutarRuta(int idarchivo)
        {
            ...
            if (idarchivo != 0)
            {
                ...
                if (archivos.Rows.Count > 0)
                {
                    ...
                    if ( !ruta.Contains("http") && !ruta.Contains("www") && ruta.Substring(0,1) != "\\")
                    {
        ...
        private void txtPendiente_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                ...
                case Keys.F10:
                        ...
                        {
                            case "temas":
                            txtTemas.Text = txtPendiente.SelectedText;
                            txtTemas.Focus();
                                break;
                            case "nombres":
                                txtNombres.Text = txtPendiente.SelectedText;
                            txtNombres.Focus();
                            break;
                            case "verbos":
                                txtVerbos.Text = txtPendiente.SelectedText;
                            txtVerbos.Focus();
                            break;
                            case "nexos":
                                txtNexos.Text = txtPendiente.SelectedText;
                            txtNexos.Focus();
                            break;
                            case "modos":
                                txtModos.Text = txtPendiente.SelectedText;
                            txtModos.Focus();
                            break;
                            case "objetos":
                                txtObjetos.Text = txtPendiente.SelectedText;
                            txtObjetos.Focus();
                            break;
                            case "archivos":
                            ...
                            break;
                            case "aplicaciones":
                                txtAplicaciones.Text = txtPendiente.SelectedText;
            ...
            switch ((e.KeyData))
            {
                ...
                case  Keys.F7:
                    ...
                    //frmClipBoard clip = new frmClipBoard();
                     string rutaRel = Utilidades.ConsultarRutasRelativas(cmbObjeto.Text );
        ...
        private void Backupeo()
        {
            ...
            String unidadDeDLL = rutaDeLaDLL.Substring(0, 2);
            ...
            origenBackupTablas = unidadDeDLL + @"\REGX\BACKUP\TABLAS\" + fecha + ".sql";
            ...
            foreach (string rutaDestino in rutas)
            {
                ...
                {
                    contenido = rutaDeHoy[0].ToString();
                    if ( !contenido.StartsWith("http") && !contenido.StartsWith("www") && !contenido.StartsWith("\\"))
                        origen = unidadDeDLL + contenido.Substring(2, contenido.Length - 2);
                    else
                        origen = contenido;
